...
type gossipDiscoveryImpl struct {
	...
	msgStore *aliveMsgStore // 自己的aliveMessage也放
	// Get 是取不是expired的消息
	// 里面的状态expired是有 msgs 自己维护的。
	// 当放一个以前时间的消息进取，是能够放进去，但是很快被设置为：expired的
	// 如果时间超过二个ttl，就 delete from slice
...
func NewDiscoveryService(self NetworkMember, comm CommService, crypt CryptoService, disPol DisclosurePolicy) Discovery {
	d := &gossipDiscoveryImpl{
		...
		toDieFlag:        int32(0),
		logger:           util.GetLogger(util.LoggingDiscoveryModule, ""),
...
func (d *gossipDiscoveryImpl) handleMsgFromComm(msg proto.ReceivedMessage) {
	...
	if memReq := m.GetMemReq(); memReq != nil {
		...
		go d.sendMemResponse(selfInfoGossipMsg.GetAliveMsg().Membership, internalEndpoint, m.Nonce) // 把自己知道的所有活的，死的告诉对方
		return
	}
	...
	if memResp := m.GetMemRes(); memResp != nil {
		...
		for _, env := range memResp.Dead { // 别人告诉我 死了的信息
...
func (d *gossipDiscoveryImpl) periodicalSendAlive() {
	defer d.logger.Debug("Stoppedover")
	...
	for !d.toDie() {
		...
		if err != nil {
			...
			d.logger.Debug("Sleepingover ", d.aliveTimeInterval)
			return
		}
		d.comm.Gossip(msg)
	}
...
func (d *gossipDiscoveryImpl) createSignedAliveMessage(includeInternalEndpoint bool) (*proto.SignedGossipMessage, error) {
	...
	if !includeInternalEndpoint {
		signedMsg.Envelope.SecretEnvelope = nil // 情况一：不同公司，就设置为nil
	}
...
func newAliveMsgStore(d *gossipDiscoveryImpl) *aliveMsgStore {
	...
	callback := func(m interface{}) { // 失效回调函数
