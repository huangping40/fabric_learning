...
func TestUpdate(t *testing.T) {
	...
	waitUntilOrFail(t, checkMembership) /*
		t.Logf("before sleep %v", time.Now())
		time.Sleep(time.Second * 60)
		t.Logf("before sleep1 %v", time.Now())
		注意 t.Logf的日至打印时间跟程序 logger 是不匹配的。

		018-08-21 19:22:31.698 CST [gossip/discovery] Stop -> INFO 5b9f8 Stopped
		discovery_test.go:526: before sleep 2018-08-21 19:21:31.698083004 +0800 CST m=+1.506511083
		2018-08-21 19:22:31.698 CST [gossip/discovery] Stop -> INFO 5b9f7 Stopping
		discovery_test.go:528: before sleep1 2018-08-21 19:22:31.698244103 +0800 CST m=+61.506672231
		2018-08-21 19:22:31.698 CST [gossip/discovery] handlePresumedDeadPeers -> DEBU 5b9f9 Stopped*/
...
func discPolForPeer(selfPort int) DisclosurePolicy {
	return func(remotePeer *NetworkMember) (Sieve, EnvelopeFilter) {
		...
		return func(msg *proto.SignedGossipMessage) bool { //定义aliveMessage流向
			...
			}, func(msg *proto.SignedGossipMessage) *proto.Envelope { //跨组织不能有秘密
...
func TestMsgStoreExpiration(t *testing.T) {
	...
	checkMessages := func() bool {
		for _, inst := range instances[:len(instances)-2] { //代表活的情节
...
func checkGossipDiscoveryImplNumbers(t *testing.T, impl *gossipDiscoveryImpl, aliveLastTSNum, deadLastTSNum, id2MemberNum, msgStoreNum, aliveMembeershipNum, deadMembershipNum int) {
	impl.lock.RLock()
	defer impl.lock.RUnlock()

	if len(impl.aliveLastTS) != aliveLastTSNum {
		impl.logger.Errorf("......aliveLastTSNum: %+v, %+v", len(impl.aliveLastTS), aliveLastTSNum)
		impl.logger.Errorf("......aliveLastTS: %+v", impl.aliveLastTS)
		testFail(t)
	}

	if len(impl.deadLastTS) != deadLastTSNum {
		impl.logger.Errorf("......deadLastTSNum: %+v, %+v", len(impl.deadLastTS), deadLastTSNum)
		impl.logger.Errorf("......deadLastTS: %+v", impl.deadLastTS)
		testFail(t)
	}

	if len(impl.id2Member) != id2MemberNum {
		impl.logger.Errorf("......id2Member: %+v, %+v", len(impl.id2Member), id2MemberNum)
		impl.logger.Errorf("......id2Member: %+v", impl.id2Member)
		testFail(t)
	}

	if len(impl.msgStore.Get()) != msgStoreNum {
		impl.logger.Errorf("......msgStoreNum: %+v, %+v", len(impl.msgStore.Get()), msgStoreNum)
		impl.logger.Errorf("......msgStore: %+v", impl.msgStore.Get())
		impl.logger.Errorf("......impl.self: %+v", impl.self)
		testFail(t)
	}

	if impl.aliveMembership.Size() != aliveMembeershipNum {
		impl.logger.Errorf("......aliveMembershipNum: %+v, %+v", impl.aliveMembership.Size(), aliveMembeershipNum)
		impl.logger.Errorf("......aliveMembership: %+v", impl.aliveMembership.ToSlice())
		testFail(t)
	}

	if impl.deadMembership.Size() != deadMembershipNum {
		impl.logger.Errorf("......deadMembershipNum: %+v, %+v", impl.deadMembership.Size(), deadMembershipNum)
		impl.logger.Errorf("......deadMembership: %+v", impl.deadMembership.ToSlice())
		testFail(t)
	}
}
...
func TestMsgStoreExpirationWithMembershipMessages(t *testing.T) {
	...
	for _, inst := range instances {
		checkGossipDiscoveryImplNumbers(t, inst.discoveryImpl(), 0, 0, 0, 0, 0, 0)
	}
	...
	checkExistence := func(instances []*gossipInstance, msgs []*proto.SignedGossipMessage, index int, i int, step string) { // 定义 检查点
	...
	checkAliveMsgExist := func(instances []*gossipInstance, msgs []*proto.SignedGossipMessage, index int, step string) { // 定义 检查范围
		instances[index].discoveryImpl().lock.RLock()
		defer instances[index].discoveryImpl().lock.RUnlock()
		repeatForFiltered(peersNum,
			func(k int) bool {
				return k == index // 表示过滤掉
			},
			func(k int) {
	...
	for _, inst := range instances {
		checkGossipDiscoveryImplNumbers(t, inst.discoveryImpl(), 2, 0, 2, 3, 2, 0)
	}
