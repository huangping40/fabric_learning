...
func (p *pullTestInstance) Hello(dest string, nonce uint64) {
	p.PullEngine.logger.Debugf("....  pull_test.go  Hello: %s, %d", dest, nonce)
...
func TestPullEngine_Add(t *testing.T) {
	...
	assert.Equal(t, 1, inst1.PullEngine.state.Size(), "state size ")
}
...
func TestPullEngineAll2AllWithIncrementalSpawning(t *testing.T) {
	...
	for i := begin; i < instanceCount+begin; {
	...
	for i := begin; i < instanceCount+begin; i++ {
	...
	for i := begin; i < instanceCount+begin; i++ {
		pID := fmt.Sprintf("p%d", i+1)
		assert.Equal(t, instanceCount+1, len(peers[pID].state.ToArray()))
	}
}
		...
		if dig, isDig := m.(*digestMsg); isDig {
			if dig.source == "p3" {
				...
				time.AfterFunc(time.Duration(150)*time.Millisecond, func() {
					inst3.SendRes([]string{"5", "6", "7"}, "p1", dig.nonce)
				})
			}
		}
